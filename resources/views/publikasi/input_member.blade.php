...
@endif
    ...
    <form action="{{route('paper.add_member_to_paper')}}" method="post" class="flex pb-10">
        @method('POST')
        @csrf
        <div class="w-1/2 space-y-4">
            <div class="space-y-2">
                ...
                <select name="member_id" value="{{old('member_id')}}" class="w-full py-2 px-4 rounded border border-black" placeholder="Member" required>
                    @foreach ($member as $m)
                        <option value="{{$m->id}}">{{$m->nama}} (NIP: {{$m->NIP}})</option>
                    @endforeach
                </select>
            </div>
            <x-role_member />
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex space-x-3">
                                <form action="{{route('paper.delete_member_from_paper', ['paper_id' => $id, 'member_id' => $d->member_id])}}" method="post">
                                    @csrf
                                    @method('DELETE')
